...
>
    ...
    <xsl:template match="/">
        <html>
            <body>
                <center>
                    <h2>Stanford CoreNLP XML Output</h2>
                </center>
                ...
                <table border="1" style="background-color:#f0f0f0;" align="center">
                    ...
                    <tr>
                        <td>
                            <xsl:if test="root/document/docId">
                                <br/>
                                <i>DocId</i>:
                                <xsl:value-of select="root/document/docId"/>
                            </xsl:if>
                            <xsl:if test="root/document/docDate">
                                <br/>
                                <i>DocDate</i>:
                                <xsl:value-of select="root/document/docDate"/>
                            </xsl:if>
                            <xsl:if test="root/document/docSourceType">
                                <br/>
                                <i>DocSourceType</i>:
                                <xsl:value-of select="root/document/docSourceType"/>
                            </xsl:if>
                            <xsl:if test="root/document/docType">
                                <br/>
                                <i>DocType</i>:
                                <xsl:value-of select="root/document/docType"/>
                            </xsl:if>
                            <xsl:if test="root/document/author">
                                <br/>
                                <i>Author</i>:
                                <xsl:value-of select="root/document/author"/>
                            </xsl:if>
                            <xsl:if test="root/document/location">
                                <br/>
                                <i>Location</i>:
                                <xsl:value-of select="root/document/location"/>
                            </xsl:if>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <th>Coreference resolution graph</th>
                    </tr>
                    <tr>
                        <td>
    ...
    <xsl:template match="root/document/sentences/sentence">
        ...
        <p>
            ...
            <xsl:if test="@sentiment">
                <xsl:text>Sentiment:</xsl:text>
                <xsl:value-of select="@sentiment"/>
            </xsl:if>
        </p>
    ...
    <xsl:template match="dep">
        <li>
            ...
            <xsl:if test="governor/@copy">^
                <xsl:value-of select="governor/@copy"/>
            </xsl:if>
            -
            <xsl:value-of select="governor/@idx"/>
            ,
            <xsl:value-of select="dependent"/>
            <xsl:if test="dependent/@copy">^
                <xsl:value-of select="dependent/@copy"/>
            </xsl:if>
            -
                    ...
                    <table border="1">
                        ...
                        <xsl:for-each select="mention">
                            <tr>
                                ...
                                <td>
                                    <xsl:if test="leftContext or rightContext">
                                        ...
